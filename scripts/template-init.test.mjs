...
describe("template:init", () => {
  ...
  it("supports a minimal preset and independently selected features", async () => {
    ...
    const siteConfig = transformContent(
      ...
      'export const siteConfig = { name: "Agent Template", shortName: "AW" };\n',
      {
  ...
  it("previews without writing and then initializes tracked text files", async () => {
    ...
    await writeFile(
      packagePath,
      '{"name":"template-agent","dependencies":{"@arlequins/env":"workspace:*"}}\n',
    );
    await writeFile(
      sstPath,
    ...
    assert.match(await readFile(packagePath, "utf8"), /template-agent/);
